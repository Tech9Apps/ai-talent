  ...
  const processFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (file.size > FILE_VALIDATION_CONFIG.maxSizeBytes) {
        throw new Error(
          `File exceeds AI analysis limit (${Math.round(
            FILE_VALIDATION_CONFIG.maxSizeBytes / 1024 / 1024
          )}MB)`
        );
      }
      ...
      updateStep(
        currentStep,
        hasAIWarnings ? "warning" : "completed",
        hasAIWarnings
      ...
      if (type === "cv") {
        ...
        updateStep(
          currentStep,
          "completed",
          `Found ${matchResult.totalMatches || 0} job matches`
        );
      } else {
      ...
      if (failingStepId) {
        ...
        if (errorMessage === "internal") {
          errorMessage =
            "AI processing failed. Please try again or use a smaller/cleaner file.";
        }
  ...
  return (
    <>
              ...
              >
                ...
                <Box sx={{ flexGrow: 1 }}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    Uploaded on{" "}
                    {cvFile.uploadedAt.toDate().toLocaleDateString()}
                  </Typography>
                </Box>
      ...
      <Dialog open={showReplaceDialog} onClose={handleCancelReplace}>
        ...
        <DialogContent>
          <DialogContentText>
            You already have a CV uploaded ({cvFile?.fileName}). Uploading a new
            CV will permanently replace the existing one and remove all its
